...
import { PurchaseReceivingPanel } from "@/components/PurchaseReceivingPanel";
...
export function ProductionInventoryWorkspace({ workspace }: { workspace: WorkspaceContext }) {
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      usage: items.reduce((total, item) => total + Math.max(0, -item.movementDelta), 0),
    };
  }, [items]);
  ...
  function openCount(item: ProductionInventoryRecord) {
    setCountTarget(item);
    setCountQuantity(item.physicalQuantity ?? Math.max(0, item.theoreticalQuantity));
    setCountNote("");
  }
  ...
  return <div className="module-page interactive-workspace production-inventory">
    <div className="module-heading"><Boxes size={24}/><div><span className="kicker">Production inventory</span><h2>Physical counts versus expected stock</h2><p>Opening theoretical stock plus append-only purchase, order-usage and correction movements creates expected remaining stock. Physical counts remain separate audit observations so variance can be investigated without assigning a cause prematurely.</p></div></div>
    ...
    <div className="production-toolbar"><div><b>{workspace.propertyName}</b><span>{workspace.city} · expected stock from opening balance + append-only movements</span></div><div><button type="button" onClick={() => void refresh()} disabled={loading}><RefreshCw size={14}/> Refresh</button>{canWrite && <button type="button" className="primary-action" onClick={() => setItemFormOpen((open) => !open)}><Plus size={14}/> New item</button>}</div></div>
    {!canWrite && <div className="settings-lock">Your authorization role is <b>{workspace.authorizationRole}</b>. Inventory records are read-only for this role; owners, managers and inventory staff can create items, receive purchases and record counts.</div>}
    {itemFormOpen && canWrite && <section className="production-task-form"><div className="settings-two"><label>Item name<input value={name} maxLength={160} onChange={(event) => setName(event.target.value)} placeholder="Paneer"/></label><label>Unit<input value={unit} maxLength={24} onChange={(event) => setUnit(event.target.value)} placeholder="kg"/></label></div><div className="inventory-create-grid"><label>Opening theoretical quantity<input type="number" min="0" step="0.001" value={theoretical} onChange={(event) => setTheoretical(Math.max(0, Number(event.target.value)))}/></label><label>Reorder quantity<input type="number" min="0" step="0.001" value={reorder} onChange={(event) => setReorder(Math.max(0, Number(event.target.value)))}/></label><label>Reference unit cost (optional)<input type="number" min="0" step="0.01" value={unitCost} onChange={(event) => setUnitCost(event.target.value)}/></label></div><div className="production-form-actions"><button type="button" onClick={() => setItemFormOpen(false)}>Cancel</button><button type="button" className="primary-action" disabled={saving || !name.trim() || !unit.trim()} onClick={() => void createItem()}>{saving ? "Creating…" : "Create item"}</button></div></section>}
    {countTarget && canWrite && <section className="production-task-form count-form"><header><div><small>New physical count</small><h3>{countTarget.name}</h3></div><span>Expected remaining: {countTarget.theoreticalQuantity}{countTarget.unit}</span></header><div className="settings-two"><label>Physical quantity<input type="number" min="0" step="0.001" value={countQuantity} onChange={(event) => setCountQuantity(Math.max(0, Number(event.target.value)))}/></label><label>Count note<input value={countNote} maxLength={500} onChange={(event) => setCountNote(event.target.value)} placeholder="Optional count context"/></label></div><div className="production-form-actions"><button type="button" onClick={() => setCountTarget(null)}>Cancel</button><button type="button" className="primary-action" disabled={saving} onClick={() => void saveCount()}>{saving ? "Recording…" : "Record count"}</button></div></section>}
    {error && <div className="form-error production-error" role="alert">{error}</div>}

    {canWrite && <PurchaseReceivingPanel workspace={workspace} onInventoryChange={refresh}/>} 

    {loading ? <div className="production-empty">Loading inventory records…</div> : items.length === 0 ? <div className="production-empty"><Scale size={24}/><b>No inventory items yet</b><span>Add the first real inventory item for this property. Munaffa does not seed production stock with fictional values.</span>{canWrite && <button className="primary-action" onClick={() => setItemFormOpen(true)}><Plus size={14}/> Add inventory item</button>}</div> : <div className="inventory-table inventory-movement-table"><div className="inventory-row inventory-movement-row production-inventory-head"><span>Ingredient</span><span>Opening</span><span>Movements</span><span>Expected</span><span>Physical</span><span>Variance</span><span>Action</span></div>{items.map((item) => { const variance = item.physicalQuantity === null ? null : Number((item.physicalQuantity - item.theoreticalQuantity).toFixed(3)); return <div className="inventory-row inventory-movement-row" key={item.id}><span><b>{item.name}</b><small>Reorder at {item.reorderQuantity}{item.unit}{item.countedAt ? ` · counted ${formatRelative(item.countedAt)}` : " · never counted"}</small></span><span>{item.openingTheoreticalQuantity}{item.unit}</span><span className={item.movementDelta < 0 ? "movement-usage" : item.movementDelta > 0 ? "movement-in" : "variance-neutral"}>{item.movementDelta > 0 ? "+" : ""}{item.movementDelta}{item.unit}</span><span><b>{item.theoreticalQuantity}{item.unit}</b></span><span>{item.physicalQuantity === null ? "Not counted" : `${item.physicalQuantity}${item.unit}`}</span><span className={variance === null ? "variance-neutral" : Math.abs(variance) >= 1 ? "variance-alert" : "variance-ok"}>{variance === null ? "—" : `${variance > 0 ? "+" : ""}${variance}${item.unit}`}</span><span>{canWrite ? <button className="inventory-count-action" onClick={() => openCount(item)}>Count</button> : <small>Read only</small>}</span></div>; })}</div>}
    <div className="demo-notice production-notice"><CircleAlert size={15}/><span>Completed orders create theoretical recipe usage; received supplier purchases create positive expected-stock movements. Neither rewrites physical stock. Variance is a review signal, not evidence of theft or waste. Physical count history is append-only.</span></div>
  </div>;
}
