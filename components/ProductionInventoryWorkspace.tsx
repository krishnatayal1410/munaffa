...
export function ProductionInventoryWorkspace({ workspace }: { workspace: WorkspaceContext }) {
  ...
  return <div className="module-page interactive-workspace production-inventory">
    <div className="module-heading"><Boxes size={24}/><div><span className="kicker">Production inventory</span><h2>Physical counts versus expected stock</h2><p>Opening theoretical stock plus append-only movements creates expected remaining stock. Physical counts remain separate audit observations so variance can be investigated without assigning a cause prematurely.</p></div></div>
    ...
    <div className="production-toolbar"><div><b>{workspace.propertyName}</b><span>{workspace.city} · production inventory records · theoretical usage from completed orders</span></div><div><button type="button" onClick={() => void refresh()} disabled={loading}><RefreshCw size={14}/> Refresh</button>{canWrite && <button type="button" className="primary-action" onClick={() => setItemFormOpen((open) => !open)}><Plus size={14}/> New item</button>}</div></div>
    ...
    <div className="demo-notice production-notice"><CircleAlert size={15}/><span>Completed orders create theoretical recipe usage; they do not rewrite physical stock. Variance is a review signal, not evidence of theft or waste. Physical count history is append-only.</span></div>
  </div>;
}
