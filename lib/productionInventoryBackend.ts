...
function requireWorkspace(context: WorkspaceContext) {
  ...
  if (!context.organizationId || !context.propertyId) {
    throw new Error("Production inventory is not initialized. Apply Supabase migrations through 006, then sign in again.");
  }
...
export async function listProductionInventory(context: WorkspaceContext): Promise<ProductionInventoryRecord[]> {
  ...
  const [{ data: items, error: itemError }, { data: counts, error: countError }] = await Promise.all([
    client
  ...
  return (items || []).map((item) => {
    ...
    return {
      ...
      theoreticalQuantity: Number(raw.theoretical_quantity || 0),
