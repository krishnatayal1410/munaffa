...
export function OnboardingWizard() {
  ...
  function finish() { saveDemoSetup({ organizationName, hospitalityType: type, role, propertyName, city, enabledModules: enabled }); router.push("/app?tour=1"); }
  ...
  return <main className="onboarding-page"><div className="onboarding-shell">
    ...
    {step === 3 && <section className="setup-step"><span className="kicker">Modules</span><h1>Choose what you want Munaffa to show first.</h1><p>You can turn everything on later.</p><div className="module-list">{modules.map((item) => { const active = enabled.includes(item); return <button key={item} className={active ? "selected" : ""} onClick={() => setEnabled((items) => active ? items.filter((x) => x !== item) : [...items, item])}><span>{active ? <Check size={15}/> : null}</span><b>{item}</b></button>; })}</div></section>}
    <footer className="setup-actions"><button className="pill ghost" disabled={step === 0} onClick={() => setStep((s) => Math.max(0, s - 1))}>Back</button><button className="pill primary" disabled={!canContinue} onClick={() => step === 3 ? finish() : setStep((s) => s + 1)}>{step === 3 ? "Open workspace" : "Continue"}</button></footer>
  </div></main>;
}
