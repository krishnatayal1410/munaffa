...
- Test migrations `005` through `012` with at least two separate organizations: users must never see another organization's tasks, inventory counts/movements, feedback, financial entries, members, invitations, properties, menu items, recipes, orders or ordering points.
...
- The Guest QR flow does **not** collect payment yet. It submits the venue's menu subtotal into the service queue; taxes, service charges, settlement and payment-provider integration remain separate until implemented and verified.
