...
14. `014_supplier_purchases.sql` — supplier directory, draft/received purchase receipts, positive inventory movements on receiving, one linked expense event, and reversing stock/finance events when management voids a receipt.
...
- Test migrations `005` through `014` with at least two separate organizations: users must never see another organization's tasks, inventory counts/movements, feedback, financial entries, bills/payments, suppliers/purchases, members, invitations, properties, menu items, recipes, orders or ordering points.
...
- Treat `inventory_movements` as append-only expected-stock events. Completed orders create negative `order_usage`; received purchases create positive `purchase` movements. Physical stock is never rewritten by either event.
- Verify order-usage idempotency by completing an order once and confirming that each recipe ingredient receives at most one `order_usage` movement for that order. Finalized orders must not be reopened through the production RPC.
...
- Treat `financial_entries` as an append-only ledger. A correction should void the original entry and create a corrected event; amount/category/source/tenant/timestamp fields cannot be rewritten.
...
- Order settlement is **recording**, not payment processing. Cash/card/UPI/bank/other means staff says that money was already received. No card authorization, UPI collect request, gateway capture or bank settlement API is performed by migration `013`.
- Verify purchasing separately: owner/manager/inventory roles may create suppliers, draft purchase receipts and receive purchases. Only owner/manager may void a receipt.
- Receiving a purchase must create positive `purchase` inventory movements and exactly one linked expense entry. Voiding a received purchase must append reversing stock movements and void the linked expense; history must not be deleted.
- A supplier receipt records the quantity and unit cost entered by staff. It does not prove invoice authenticity, delivery quality or bank settlement.
...
- Test the purchasing loop: create supplier → create draft receipt with real inventory items → receive it → verify expected stock increases and Profit records one inventory-purchase expense → void as owner/manager → verify reversing stock movements and the expense becomes voided.
- Public guest-order abuse controls are guardrails, not a complete anti-abuse system. Add edge/WAF rate limiting before high-volume public rollout.
...
- **Inventory** manages real `inventory_items`, append-only physical `inventory_counts`, expected stock calculated as opening theoretical quantity plus append-only inventory movements, and supplier purchase receiving. Received purchases increase expected stock and create linked expenses. Variance is physical count minus expected stock and is never labelled theft automatically.
- **Guests** captures real `guest_feedback` and role-protected recovery status scoped to the active property.
- **Profit** combines the append-only financial ledger with order billing/settlement and supplier-purchase expenses. Captured recorded settlements create linked revenue events; received purchases create linked expense events. Recorded contribution remains operational, not an audited P&L.
