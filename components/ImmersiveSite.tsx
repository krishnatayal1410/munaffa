...
import { ArrowDown, ArrowRight, Camera, CircleDollarSign, UtensilsCrossed } from "lucide-react";
...
const chapters = [
  ...
  { eyebrow: "03 / THE TABLE", title: <>A table can become<br/><em>a live touchpoint.</em></>, body: "Ordering should feel like part of the venue. Try the working interaction while the 3D camera remains inside the dining environment." },
  ...
  { eyebrow: "09 / MUNAFFA", title: <>Experience first.<br/><em>Operations underneath.</em></>, body: "The new foundation combines cinematic storytelling with working interactions so the eventual product feels like one system rather than a marketing site bolted onto an app." }
];
...
export function ImmersiveSite() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        ...
        onUpdate: (self) => update({ progress: self.progress, velocity: self.getVelocity() })
      });
      ...
      sections.forEach((section, index) => {
        ...
        if (!reduced) {
          ...
          if (reveal.length) gsap.fromTo(reveal,
            { opacity: 0, y: 42, filter: "blur(12px)" },
            { opacity: 1, y: 0, filter: "blur(0px)", stagger: 0.07, ease: "none", scrollTrigger: { trigger: section, start: "top 80%", end: "top 42%", scrub: 1 } }
          );
        }
      });
    }, root);
  ...
  return <main ref={root} className={`experience scene-${activeScene}`} onPointerMove={pointerMove}>
    ...
    <Header/>
    <ProgressRail scene={activeScene}/>
    ...
    <div id="experience-story" className="story">
      {chapters.map((chapter, index) => <section id={`chapter-${index}`} data-scene={index} className={`chapter ${index % 2 ? "chapter-right" : "chapter-left"}`} key={chapter.eyebrow}>
        <div className="chapter-copy">
          ...
          {index === 2 && <MenuExperience/>}
          {index === 3 && <div className="product-caption" data-reveal><UtensilsCrossed size={15}/><span>Three real product images float in WebGL space while the camera passes through the culinary chapter.</span></div>}
          {index === 4 && <ARLens/>}
          {index === 6 && <ProfitLens/>}
          ...
          {index === 8 && <div className="final-actions" data-reveal><a href="#chapter-2" className="primary-action">Try the interactions <ArrowRight size={14}/></a><a href="#chapter-0" className="secondary-action">Replay</a></div>}
        </div>
        <span className="chapter-index" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span>
      </section>)}
    </div>

    <footer className="site-footer"><Brand/><p>Immersive Munaffa concept · interactions are functional prototype experiences.</p><div><a href="/privacy">Privacy</a><a href="/terms">Terms</a><a href="#chapter-0">Top ↑</a></div></footer>
  </main>;
}

function Header() {
  return <header className="floating-header"><Brand/><nav><a href="#chapter-2">Menu</a><a href="#chapter-4"><Camera size={12}/> AR Lens</a><a href="#chapter-6"><CircleDollarSign size={12}/> Profit</a></nav><a className="enter-link" href="#chapter-1">ENTER <ArrowRight size={12}/></a></header>;
}

function Brand() {
  return <a className="brand" href="#chapter-0"><i>M</i><span>MUNAFFA</span></a>;
}

function ProgressRail({ scene }: { scene: number }) {
  return <aside className="progress-rail" aria-label="Experience chapters">{chapters.map((chapter, index) => <a key={chapter.eyebrow} href={`#chapter-${index}`} className={index === scene ? "active" : index < scene ? "passed" : ""} aria-label={`Go to ${chapter.eyebrow}`}><i/></a>)}</aside>;
}
